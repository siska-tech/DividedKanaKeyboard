...
impl<'d, P: PacketPool> ConnectionManager<'d, P> {
            ...
            + crate::ControllerCmdSync<bt_hci::cmd::link_control::Disconnect>,
    {
        ...
        match _event {
            crate::security_manager::SecurityEventData::SendLongTermKey {
                handle,
                random_number,
                ...
                if let Some((conn, identity)) = conn_info {
                    if random_number != [0; 8] || encrypted_diversifier != 0 {
                        warn!("[host] Long term key request is not LE Secure Connections");
