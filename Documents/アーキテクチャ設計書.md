...
> **実装状況（2026-06-12）**: 本書が記す L0〜L4 は **実装済み・実機動作**
> （BLE #07 は feature "ble" でコード実装済み・主要実機検証済み, §14.3）。
...
| `core/config.rs`     | –      | 設定の中間表現 IR v2（パラメータ＋全カテゴリ差分＋カスタムレイヤー）＋serialize/parse（#12, CRC16）。値タグ Kana/Keys/**Hold**・擬似 usage MO(n)/TG(n)（fw 0.3）  | ✅      |
| `core/build.rs`      | –      | YAML→phfコード生成（LAYERS/COMBO2/COMBO3/PREFIX2/MODE_*/MATRIX/SHIFT_KEYS/SINGLE_TAP_SCS ＋ **PHYS_\*/LAYOUT_\***（全物理キー・形状, fw 0.3）。実行時パース無し） | ✅      |
...
| `fw/usb_config.rs`   | L4     | PC↔端末 設定チャネル（#12, vendor HID/IF1, INFO/READ/WRITE/COMMIT/RESET/REBOOT ＋ fw 0.3: READ_MATRIX_FULL/READ_LAYOUT/SEC_CUSTOM/capability ビット）             | ✅ 実機 |
| `fw/usb_hid.rs`      | L4     | HID キーボードレポート組立（IF0, 1000Hz）                                                                                                                         | ✅ 実機 |
| `fw/ble.rs`          | L4     | BLE-HID（HID over GATT, #07。cyw43+trouble-host。feature "ble"・左手番のみ・USB優先/BLEフォールバック, §14.3）                                                     | ✅ 主要実機 |
| `fw/main.rs`         | L1     | 役割判定(USB列挙)＋EE_HANDS＋設定適用＋6タスク配線＋LED ＋ fw 0.3: 直接キー `Direct` 分類・ホールド保持・レイヤー状態機械（パススルー/カスタム, 詳細設計 G）      | ✅ 実機 |
...
> - **fw 0.3（2026-06-12, naginata-config #05-future-work 実装分）**: 修飾キーホールド（TAG_HOLD）・
>   QWERTY パススルー/任意カスタムレイヤー（擬似 usage MO(n)/TG(n)・SEC_CUSTOM・透過既定）・
>   全物理キー/形状の実機読み出し（READ_MATRIX_FULL/READ_LAYOUT、実機採取データ記入済み）・
>   INFO capability ビット。詳細は詳細設計書 G 章と `naginata-config/docs/02-protocol.md`。
...
- **プロトコル**: vendor-defined HID（IF1, usagePage 0xFF00, 64B）。`INFO/READ_DEFAULTS/READ_OVERRIDES/WRITE/COMMIT/RESET/REBOOT/READ_PARAMS/WRITE_PARAMS/READ_MATRIX/READ_LASTKEY/READ_MATRIX_FULL/READ_LAYOUT`。
  v2 ワイヤ互換の拡張（fw 0.3, #05-future-work 実装分）: INFO capability ビット（payload[16]）＋
  maxCustomLayers（[17]）、値タグ `TAG_HOLD(2)`（押しっぱなし保持）、擬似 usage
  `0xF0|n`=MO(n)/`0xF8|n`=TG(n)（QWERTY パススルー＋カスタムレイヤー SEC 6。透過既定、
  params flags bit0=IME 自動切替）。仕様の正は設定ツール側 `naginata-config/docs/02-protocol.md`。
...
- **対象カテゴリ（IR v2）**: 単打/シフト面 layers・combo2・combo3・編集モード modes・未使用物理キーの直接割当 extra・カスタムレイヤー custom（空なら IR に書かない＝ダウングレード互換）・パラメータ（窓/tap/リピート/LED輝度/flags）。
- **learn-key**: `READ_LASTKEY` が最後に押された物理位置を返し、未使用キーへの直接割当（フェーズ3）を支援。

### 14.3 BLE-HID（#07 / FR-5。実装済み・主要実機検証済み）

`cargo build --release --features ble` で有効化（既定ビルドは従来の USB のみ＝回帰ゼロ）。
実装は `fw/ble.rs`（cyw43 0.7 + trouble-host 0.6 / HID over GATT）。

```mermaid
flowchart LR
  eng[engine_task] --> rep[REPORTS ch]
  rep --> disp{usb_task hid_fut<br/>USB列挙中?}
  disp -->|Yes USB優先| usb[HidWriter IF0]
  disp -->|No| bch[BLE_REPORTS ch]
  bch --> ble[ble.rs<br/>Input Report notify]
```

- **役割（D.1 の拡張）**: BLE マスタになれるのは**左手番のみ**。起動 3 秒以内に USB 列挙が
  無ければ広告開始。マスタ判定は `is_master() = USB_CONFIGURED || BLE_STATE != 0`。
  以降 USB が挿されれば出力だけ USB へ切替（FR-5.2: USB優先/BLEフォールバック）。
- **リソース**: CYW43 は PIO1(sm0)＋DMA_CH2＋GP23/24/25/29（WS2812 の PIO0/DMA_CH0 と非衝突）。
  firmware blob は cyw43-firmware crate、NVRAM は embassy の `nvram_rp2040.bin` を同梱。
- **GATT**: HID Service（Report Map は BLE 用 63B。Boot Keyboard Input + LED Output）＋ Battery ＋
  Device Information（PnP ID = USB 0xc0de:0xcafe）。ペアリングは Just Works。
- **Bond store**: 末尾から3番目の4KBセクタに 1 ホスト分の local Static Random Address /
  peer identity / IRK / LTK / security level / CCCD を保存する。
- **LED（FR-6）**: USBマスタ緑 / BLE接続マゼンタ / BLE広告中 暗マゼンタ / ペアリング済みシアン /
  失敗詳細色 / スレーブ青。
- **依存上の注意**: embassy-sync は trouble に合わせ 0.7 固定（gatt マクロの mutex 型の版一致）。
  ARMv6-M の CAS 非対応は portable-atomic(critical-section) で解決。
- **残**: モバイルバッテリ給電での左右動作最終確認、iOS/iPadOS 追加確認、複数ホスト切替、
  省電力（NFR-6）。詳細は [issues/07](../issues/07-ble-hid.md)。
